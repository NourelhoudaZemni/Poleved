            ...
            <div class="sm:flex items-center justify-between">
                ...
                <div>
                    <button
                        ...
                        <a href="{{ route('location.create') }}" class="text-sm font-medium leading-none text-white">Add New</a>
                    </button>
                </div>
            </div>
        </div>
        <div class="bg-white shadow px-4 md:px-10 pt-4 md:pt-7 pb-5 overflow-y-auto">
            <table class="w-full whitespace-nowrap">
                ...
                <tr tabindex="0" class="focus:outline-none h-16 w-full text-sm leading-none text-gray-800  ">
                    <th class="font-normal text-left pl-12">user</th>
                    <th class="font-normal text-left pl-12">velo</th>
                    <th class="font-normal text-left pl-20">duree</th>
                </tr>
                </thead>
                <tbody class="w-full">
                @if(count($data) > 0)
                    @foreach($data as $row)
                        <tr tabindex="0"
                            ...
                            <td class="pl-12">
                                <p class="text-sm font-medium leading-none text-gray-800">{{ $row->user }}</p>
                            </td>
                            <td class="pl-12">
                                <p class="font-medium">{{ $row->velo }}</p>
                            </td>
                            <td class="pl-20">
                                <p class="font-medium">{{ $row->duree }}</p>
                            </td>
                            ...
                            <td class="px-7 2xl:px-0">
                                <form class="flex" method="post" action="{{ route('location.destroy', $row->id) }}">
                                    @csrf
                                    @method('DELETE')
                                    <br>
                                    <div tabindex="0"
