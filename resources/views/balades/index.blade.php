            ...
            <div class="sm:flex items-center justify-between">
                ...
                <div>
                    <button
                        ...
                        <a href="{{ route('balades.create') }}" class="text-sm font-medium leading-none text-white">Add New
                            Balade</a>
                    </button>
                </div>
            </div>
        ...
        <div class="bg-white shadow px-4 md:px-10 pt-4 md:pt-7 pb-5 overflow-y-auto">
            <table class="w-full whitespace-nowrap">
                ...
                <tr tabindex="0" class="focus:outline-none h-16 w-full text-sm leading-none text-gray-800  ">
                    ...
                    <th class="font-normal text-left pl-4">Description</th>
                    ...
                    <th class="font-normal text-left pl-4">Discount</th>
                </tr>
                </thead>
                <tbody class="w-full">
                @if(count($data) > 0)
                    @foreach($data as $row)
                        <tr tabindex="0"
                            ...
                            <td class="pl-4">
                                <p class="text-sm font-medium leading-none text-gray-800">{{ $row->title }}</p>
                            </td>
                            <td class="pl-4">
                            ...
                            <td class="pl-4">
                                <p class="font-medium">{{ $row->price }}</p>
                            </td>
                            <td class="pl-4">
                                <p class="font-medium">{{ $row->quantity }}</p>
                            </td>
                            <td class="pl-4">
                                <p class="font-medium">{{ $row->discount_price }}</p>
                            </td>
                            {{--                    <td class="pl-16">--}}
                            {{--                        <div class="flex items-center">--}}
                            ...
                            <td class="px-7 2xl:px-0">
                                <form class="flex" method="post" action="{{ route('balades.destroy', $row->id) }}">
                                    @csrf
                                    @method('DELETE')
                                    <br>
                                    <div tabindex="0"
                                        ...
                                        <a href="{{ route('balades.show', $row->id) }}">View</a>
                                    </div>
                                    <div tabindex="0"
                                         class="text-center focus:outline-none focus:text-indigo-600 text-xs w-full hover:bg-indigo-700 py-4 px-4 cursor-pointer hover:text-white">
                                        <a href="{{ route('balades.edit', $row->id) }}">Edit</a>
                                    </div>
                                    <div tabindex="0"
                                         class="text-center focus:outline-none focus:text-indigo-600 text-xs w-full hover:bg-indigo-700 py-4 px-4 cursor-pointer hover:text-white">
                ...
                @else
                    <tr>
                        <td colspan="5" class="text-center">No balades Found</td>
                    </tr>
                @endif
                </tbody>
            </table>
